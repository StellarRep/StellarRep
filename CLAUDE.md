...
**Post-Phase-5: split into the multi-repo org layout** described above. `git subtree split --prefix=<dir> -b split-<dir>` on the monorepo, pushed each resulting branch to `main` on the new repo — full history preserved, not squashed. This umbrella repo had `contracts/`, `app/`, `docs/` removed after the split. Local clones for ongoing work live side by side under `~/Development/drips.repo/StellarRep-org/{StellarRep,contracts,app,docs}` (siblings, each its own checkout — not nested).

**Phase 6 — done, in `StellarRep/app`.** `ProfileView`/`ProfileViewModel`: look up any address (defaults to the current wallet's own), aggregate rating + job count + review list; looking up an unregistered address is a distinct state (`isNotRegistered`), not a generic error — `RegistrationView` (the View Phase 5 explicitly deferred) is embedded right there when it's your own address, since only that address can call `register_worker`. `SubmitReviewView`/`SubmitReviewViewModel`: the reviewer-side flow — paste worker address + job ID, pick a 1–5 rating, sign and submit; self-review caught client-side before ever reaching the network. Handoff mechanism decision: manual paste for MVP, not QR/deep-link — filed as a scoped v2 issue, [`StellarRep/app#1`](https://github.com/StellarRep/app/issues/1), per the roadmap's own suggestion to defer alternatives explicitly rather than leave the choice vague. `RootView`/`MainTabView` are the app's actual entry point now — `StellarRepApp` constructs only `RootView()`, which owns the one `OnboardingViewModel` (needs to observe `.ready` itself) and switches between `OnboardingView` and `MainTabView`; `OnboardingView` went from `public` back to internal accordingly, taking its view model via init instead of owning it. 21 tests pass total (14 from Phase 4/5 plus 7 new): `ProfileViewModelTests`/`SubmitReviewViewModelTests` use a new `Support/StubReputationService.swift` (Result-based, resolves immediately) rather than `RegistrationViewModelTests`'s continuation-based mock, since these prove response *mapping* (which error becomes which UI state) rather than the pending transition itself. Verified via `swift build`, `swift test`, and iOS Simulator for both schemes. Next: **Phase 7 — Hardening, Docs, Demo Data**, per `docs/ROADMAP.md` (in the `StellarRep/docs` repo). Update this section as phases complete so anyone (human or Claude) picking this repo back up knows where to resume without re-reading everything.
